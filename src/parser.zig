...
pub const ParseError = error{
    ...
    ExpectedFuncType,
    /// Invalid Opcode found while parsing the instruction.
    InvalidOpcode,
...
fn Parser(comptime ReaderType: type) type {
    return struct {
        ...
        fn parseModule(self: *Self, gpa: Allocator) Error!wasm.Module {
            ...
            const version = try self.reader.reader().readInt(u32, .little);
            ...
            while (self.reader.reader().readByte()) |section_id| {
                ...
                switch (@as(wasm.Section, @enumFromInt(section_id))) {
                    .custom => {
                    ...
                    .code => {
                        ...
                        for (try readVec(&module.code.data, reader, gpa)) |*code| {
                            ...
                            const code_reader = code_limited_reader.reader();
...

                            {
                                ...
                                while (code_reader.readEnum(std.wasm.Opcode, .little)) |opcode| {
                                    const instr = try buildInstruction(opcode, gpa, code_reader);
                                    try instructions.append(instr);
                                } else |err| switch (err) {
                                    error.EndOfStream => {
                                        const maybe_end = instructions.pop() orelse return error.MissingEndForBody;
                                        if (maybe_end.opcode != .end) return error.MissingEndForBody;
                                    },
                                    error.InvalidValue => @panic("Invalid value"),
                                    // else => |e| {
                                    //     std.log.scoped(.wasmparser).err("Error while parsing instruction: {s} at {}", .{ @errorName(e), self.reader.bytes_read });
                                    //     return e;
                                    // },
                                }
...
fn ElementType(comptime ptr: type) type {
    return std.meta.Elem(std.meta.Child(ptr));
}
...
fn readLeb(comptime T: type, reader: anytype) !T {
    ...
    const is_signed_int = t_type_info == .int and t_type_info.int.signedness == .signed;
...
fn readEnum(comptime T: type, reader: anytype) !T {
    switch (@typeInfo(T)) {
        .@"enum" => |enum_type| return @enumFromInt(try readLeb(enum_type.tag_type, reader)),
...
fn readInit(reader: anytype) !wasm.InitExpression {
    ...
    const init: wasm.InitExpression = switch (@as(std.wasm.Opcode, @enumFromInt(opcode))) {
...
fn buildInstruction(opcode: std.wasm.Opcode, gpa: Allocator, reader: anytype) !wasm.Instruction {
    ...
    instr.value = switch (opcode) {
        ...
        .call,
        // ref.func 'x'
        @as(std.wasm.Opcode, @enumFromInt(0xD2)),
        .local_get,
        .local_set,
        .local_tee,
        .global_get,
        ...
        // ref.null 't'
        @as(std.wasm.Opcode, @enumFromInt(0xD0)) => .{ .reftype = try readEnum(wasm.RefType, reader) },
        // select 'vec(t)'
        @as(std.wasm.Opcode, @enumFromInt(0x1C)) => blk: {
            const len = try readLeb(u32, reader);
        ...
        .misc_prefix => @as(wasm.Instruction.InstrValue, blk: {
            const misc = try readEnum(std.wasm.MiscOpcode, reader);
            instr.extra = .{ .misc = misc };
            switch (misc) {
                .i32_trunc_sat_f32_s,
                .i32_trunc_sat_f32_u,
                .i32_trunc_sat_f64_s,
        ...
        .simd_prefix => @as(wasm.Instruction.InstrValue, blk: {
            const simd = try readEnum(std.wasm.SimdOpcode, reader);
            instr.extra = .{ .simd = simd };
            switch (simd) {
                else => break :blk .{ .u32 = try readLeb(u32, reader) },
            }
        }),
        .atomics_prefix => @as(wasm.Instruction.InstrValue, blk: {
            const atomics = try readEnum(std.wasm.AtomicsOpcode, reader);
            instr.extra = .{ .atomics = atomics };
            break :blk .{ .u32 = try readLeb(u32, reader) };
        }),
        .i32_const => .{ .i32 = try readLeb(i32, reader) },
        .i64_const => .{ .i64 = try readLeb(i64, reader) },
        .f32_const => .{ .f32 = @bitCast(try readLeb(u32, reader)) },
        .f64_const => .{ .f64 = @bitCast(try readLeb(u64, reader)) },
