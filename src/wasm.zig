...
pub const ValueType: type = MergedEnums(NumType, RefType);
...
pub const Section: type = MergedEnum(wasm.Section, &.{
...
fn MergedEnum(comptime T: type, comptime fields: []const TypeInfo.EnumField) type {
    ...
    std.mem.copyForwards(TypeInfo.EnumField, &new_fields, old_fields);
    std.mem.copyForwards(TypeInfo.EnumField, new_fields[old_fields.len..], fields);
...
pub const BlockType: type = MergedEnum(ValueType, &.{.{
    .name = "block_empty",
    .value = @intFromEnum(wasm.BlockType.empty),
}});
...
pub const table_get: wasm.Opcode = @enumFromInt(0x25);
pub const table_set: wasm.Opcode = @enumFromInt(0x26);
